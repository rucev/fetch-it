<script setup lang="ts">
  ...
  const copyToClipboard = async () => {
    ...
    try {
      await navigator.clipboard.writeText(curlRef.value.innerText)
      isCopied.value = true
...
<template>
  <div v-if="props.curl" class="fixed inset-0 h-screen w-screen flex items-center justify-center z-40" role="dialog" aria-modal="true" aria-labelledby="curl">
    <div class="bg-stone-700 text-gray-50 max-h-4/5 min-w-1/2 w-fit max-w-4/5 flex flex-col shadow-2xl rounded-sm py-2 px-3">
      ...
      <div class="self-end fixed mt-12 mr-2 flex flex-col items-center group" aria-live="assertive">
        ...
        <div class="fixed mt-8 flex-col items-center hidden mb-5 group-hover:flex">
          ...
          <span class="-mt-2 relative rounded-md z-10 p-4 text-xs leading-none text-stone-50 whitespace-no-wrap bg-stone-950 shadow-lg">
